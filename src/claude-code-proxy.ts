...
Endpoints:
  ...
  const Port:number = (() => {
    if (PortFlagIndex < 0) { return Number(process.env["PORT"] ?? 3000) }
...
/**** content block types (subset of the Anthropic Messages API) ****/
  ...
  type MessageContent = string | ContentBlock[]   // plain string or block array
  ...
  interface ChatCompletionRequest {
    ...
    max_tokens?:number
    temperature?:number
    session_id?:string // custom extension: session id for conversation continuity
  }
...
// NDJSON event as Claude Code expects it via stdin
...
/**** normalizedContent ****/
  ...
  function normalizedContent (Content:MessageContent):ContentBlock[] {
...
/**** MessagesAsNDJSON ****/
  ...
  function MessagesAsNDJSON (Messages:Message[]):string {
    return Messages
      .filter((Msg) => (Msg.role !== "system"))
      .map((Msg):ClaudeStreamEvent => ({
        type: (Msg.role === "user") ? "user" : "assistant",
        message:{ role:Msg.role as ("user"|"assistant"), content:normalizedContent(Msg.content) },
      }))
...
/**** extractedSystemPrompt ****/
  ...
  function extractedSystemPrompt (Messages:Message[]):string|undefined {
    const SystemMsg = Messages.find((Msg) => (Msg.role === "system"))
    if (SystemMsg == null) { return undefined }

    if (typeof SystemMsg.content === "string") { return SystemMsg.content }
    return (SystemMsg.content as ContentBlock[])   // content array: merge all text blocks
...
/**** ContentAsString ****/
  ...
  function ContentAsString (Content:RawContent):string {
...
/**** TextExtractedFromStreamJSON ****/
  ...
  function TextExtractedFromStreamJSON (Raw:string):string {
    return Raw.split("\n")
      ...
      .reduce((LastText, Line) => {
        try {
          ...
          if ((Event.type === "assistant") && (Event.message?.content != null)) {
            const Text = ContentAsString(Event.message.content as RawContent)
...
/**** ResponseFromClaudeCode ****/
  ...
  async function ResponseFromClaudeCode (
    NDJSON:string, SystemPrompt?:string, SessionId?:string
  ):Promise<ClaudeResult> {
    const Args:string[] = [
      "--print", "--verbose", "--input-format", "stream-json",
      "--output-format", "stream-json", "--dangerously-skip-permissions",
    ]
    if (SystemPrompt != null) { Args.push("--system-prompt", SystemPrompt) }

    const UsedSessionId = SessionId ?? randomUUID()
    Args.push("--session-id", UsedSessionId)

    const { promise, resolve, reject } = Promise.withResolvers<ClaudeResult>()
...
/**** ResponseFromClaudeCodeStreaming ****/
  ...
  async function ResponseFromClaudeCodeStreaming (
    NDJSON:string, onToken:(Token:string) => void,
    SystemPrompt?:string, SessionId?:string
  ):Promise<string> {
    const Args:string[] = [
      "--print", "--verbose", "--input-format", "stream-json",
          ...
          try {
            ...
            if ((Event.type === "assistant") && (Event.message?.content != null)) {
              ...
              if (Text.length > LastText.length) {
                onToken(Text.slice(LastText.length)); LastText = Text
              }
            }
...
/**** buildChatResponse ****/
  ...
  function buildChatResponse (Content:string, Model:string, SessionId:string) {
    const now = Date.now()
    return {
      id:          `chatcmpl-${now}`,
...
/**** buildTextResponse ****/
  ...
  function buildTextResponse (Text:string, Model:string) {
    const now = Date.now()
    return {
      id:      `cmpl-${now}`,
...
/**** sendError ****/
  ...
  function sendError (Response:http.ServerResponse, Status:number, Msg:string):void {
...
/**** handleChatCompletions ****/
  ...
  async function handleChatCompletions (
    ...
    if (Body.stream) {
      ...
      try {
        await ResponseFromClaudeCodeStreaming(
          NDJSON, (Token) => Response.write(SSEChunk(Token, Model)),
          SystemPrompt, SessionId
        )
      } catch (Signal) {
        console.error("[chat/stream]", Signal)
        Response.write(SSEChunk(
          `\n\n[Proxy error: ${(Signal as Error).message}]`, Model
        ))
      }
    ...
    try {
      ...
      sendError(Response, 502, `Claude Code error: ${(Signal as Error).message}`)
    }
  }
...
/**** assertThatClaudeIsInstalled ****/
  ...
  async function assertThatClaudeIsInstalled():Promise<void> {
...
/**** assertThatClaudeIsAuthenticated ****/
  ...
  async function assertThatClaudeIsAuthenticated():Promise<void> {
    const { promise, resolve } = Promise.withResolvers<void>()
